     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "在「📈 條件統計」工作表的訂單資料上，按 ⌘+T 轉為表格，再插入樞紐分析表"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "把「業務員」拖到列區域，「金額」拖到值區域 → 看到各業務員的總金額"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "加上「區域」到欄區域 → 變成交叉表：每位業務員在各區域的業績"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "在值欄位設定中，把「加總」改成「計數」→ 看出每人的訂單筆數"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "對日期欄位右鍵 → 群組 → 依月份群組 → 看到月度彙總"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "新增計算欄位：平均單價 = 金額 / 計數。觀察哪位業務員的單價最高"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "插入交叉分析篩選器，用「付款狀態」篩選 → 對比已付/未付的業績分佈"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "建立第二個樞紐表：產品類別 × 區域的金額矩陣，並讓兩個樞紐共用同一個篩選器"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "在「📊 基礎函式」的業績資料上，選取整個月份欄 → 條件式格式化 → 資料橫條"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "選取同一欄 → 條件式格式化 → 色階 → 觀察紅綠漸層代表高低"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "設定醒目提示：業績 > 60000 標為綠色底，< 40000 標為紅色底"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "使用「頂端/底端規則」→ 標記前 3 名業績為粗體金色"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "自訂公式規則：=MOD(ROW(),2)=0 → 隔列加底色（斑馬紋效果）"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "自訂公式規則：=$E2>50000 → 整列（不是單格）標色。注意 $ 的位置！"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "用圖示集（紅黃綠燈號）標記績效分數：≥90 綠燈、≥60 黃燈、其他紅燈"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "自訂公式：=COUNTIF($A:$A,$A2)>1 → 標記重複的姓名。再試試用在訂單資料找重複"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "在空白儲存格建立下拉選單：業務, 財務, 資訊, 人事（逗號分隔直接輸入）"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "建立整數驗證：限制只能輸入 1~100 的分數，並設定錯誤提醒訊息"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "建立日期驗證：限制只能輸入今天以後的日期（公式：=TODAY()）"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "在另一個儲存格建立「文字長度 = 10」的驗證 → 模擬手機號碼限制"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "用命名範圍做下拉選單：先把部門清單命名為「部門列表」→ 驗證來源填 =部門列表"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "建立連動下拉：主選單選「區域」→ 子選單用 INDIRECT 顯示該區域的業務員"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "自訂公式驗證：=AND(LEN(A2)=10, LEFT(A2,1)=\"0\") → 限制只能輸入 0 開頭的 10 碼"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "在已有資料的欄位加上驗證後 → 資料 → 圈選無效資料 → 找出不符規則的舊資料"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "選取「📊 基礎函式」的業績資料 → 插入直條圖 → 觀察自動產生的圖表"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "把同一份資料改成折線圖 → 對比直條 vs 折線，哪個更適合看趨勢？"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "刪除不必要的格線和框線 → 加上資料標籤 → 讓數字直接顯示在圖上"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "修改標題：從「業績」改成有觀點的標題，如「Q2 業績成長 15%，張志偉持續領先」"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "建立組合圖：左軸=金額（直條），右軸=成長率（折線），用在不同量級的資料"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "在每位業務員旁邊加走勢圖（Sparklines）：插入 → 走勢圖 → 折線"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "做一張「儀表板風格」圖表：只用 2 個顏色、無格線、有觀點標題、字體一致"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "嘗試建立瀑布圖（Waterfall Chart）來展示收入和支出的累計效果"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "在「📊 基礎函式」選取業績資料的金額欄 → 在名稱方塊輸入「月薪」→ 測試 =SUM(月薪)"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "選取任意資料範圍 → ⌘+T 轉為表格 → 觀察公式欄自動變成結構化參照"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "用 ⌘+⇧+F3 → 依選取範圍的欄標題自動建立多個命名範圍 → 公式→名稱管理員檢查"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "在表格中新增一列資料 → 觀察公式和範圍是否自動擴展"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "寫出結構化參照公式：=SUMIFS(銷售表[金額], 銷售表[區域], \"北區\")"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "用 @ 符號：在表格內某列寫 =[@金額]*1.1 → 觀察公式自動填充到整欄"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "建立動態命名範圍：公式→定義名稱→參照用 OFFSET+COUNTA 讓範圍隨資料自動增長"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "用命名範圍做表格間的跨表引用：Sheet2 的公式直接用名稱而非 Sheet1!A1:A10"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "選取要讓使用者編輯的儲存格 → 右鍵 → 儲存格格式 → 保護 → 取消「鎖定」"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "校閱 → 保護工作表 → 設一個簡單密碼 → 測試：公式格不能改，輸入格可以改"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "選取含公式的儲存格 → 勾選「隱藏」→ 再保護工作表 → 觀察公式欄是否顯示空白"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "校閱 → 保護活頁簿結構 → 測試：嘗試新增/刪除工作表，應該被擋住"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "取消保護 → 重新設定：允許排序和篩選，但不允許修改格式和插入欄列"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "檔案 → 另存新檔 → 工具 → 一般選項 → 設定「開啟密碼」和「修改密碼」"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "綜合保護：建立一張「客戶填寫用」的表格，公式全鎖定+隱藏，輸入格解鎖，設下拉選單，保護工作表。測試別人能不能正常填寫又不能改公式"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "搭配 VBA 保護：用 VBA 在開啟檔案時自動保護工作表，關閉時解除保護（Workbook_Open / BeforeClose 事件）"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "建立一個簡單公式：利潤 = 售價 × 數量 - 成本。然後手動改售價觀察利潤變化"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "資料 → 假設分析 → 目標搜尋：讓利潤 = 100000，變更售價 → 看 Excel 反推出多少"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "建立單變數資料表：左欄放 5 種不同利率 → 看每種利率下的月付款金額"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "資料 → 假設分析 → 分析藍本：建立「樂觀/中性/悲觀」三種情境並切換觀察"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "建立雙變數資料表：左欄=利率（3%~7%），上列=年期（10~30年）→ 交叉矩陣"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "在分析藍本中 → 摘要報表 → 自動產生三種情境的對比表"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "綜合運用：建立一個定價模型，用目標搜尋找損益兩平點，用資料表做敏感度分析"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "嘗試規劃求解（Solver）：在限制條件下最佳化結果（需先啟用增益集）"
       ]
      ]
     }
    ]
     ...
     },
     {
      ...
      "items": [
       ...
       [
        ...
        "□",
        "🟢 暖身",
        "資料 → 取得資料 → 從表格/範圍 → 進入 Power Query 編輯器 → 觀察介面"
       ],
       [
        2,
        "□",
        "🟢 暖身",
        "在 PQ 編輯器中：移除空白列 → 變更欄位類型（文字/數字/日期）→ 關閉並載入"
       ],
       [
        3,
        "□",
        "🔵 標準",
        "匯入一個 CSV 檔案 → 在 PQ 中：分割欄位（依分隔符號）→ 移除不需要的欄位"
       ],
       [
        4,
        "□",
        "🔵 標準",
        "新增自訂欄位：用 PQ 公式新增「含稅金額 = [金額] * 1.05」"
       ],
       [
        5,
        "□",
        "🟡 變化",
        "合併查詢：把兩個表格依共同欄位（如產品代碼）做 Left Join 合併"
       ],
       [
        6,
        "□",
        "🟡 變化",
        "附加查詢：把兩個相同結構的月報表上下合併（垂直合併）"
       ],
       [
        7,
        "□",
        "🔴 挑戰",
        "從資料夾匯入：把多個 Excel 檔案放同一個資料夾 → 取得資料 → 從資料夾 → 自動合併"
       ],
       [
        8,
        "□",
        "🔴 挑戰",
        "建立參數化查詢：讓使用者可以在 Excel 中輸入月份 → PQ 自動篩選該月資料"
       ]
      ]
     }
    ]
