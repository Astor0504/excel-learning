export const LESSON_DOWNLOADS = {
  "P1-02": {
    "title": "基礎函式練習資料",
    "intro": "這份資料適合直接練 SUM、AVERAGE、COUNT、COUNTA、MAX、MIN、MEDIAN、LARGE、SMALL。先用同一張表把最常用統計與排名函數練熟，後面所有報表課都會順很多。",
    "files": [
      {
        "label": "基礎函式資料",
        "description": "6 個月份的員工業績表，適合練加總、平均、計數、中位數、最大值、最小值與第 N 大 / 小。",
        "href": "practice-data/p1-02-basic-functions.csv",
        "format": "CSV",
        "fileName": "p1-02-basic-functions.csv"
      }
    ]
  },
  "P1-03": {
    "title": "條件判斷練習資料",
    "intro": "這份資料用來練 IF、IFS、AND / OR 與 SWITCH 的判斷邏輯。很適合拿來做績效等級、出勤提醒、部門代碼對應和薪資判斷。",
    "files": [
      {
        "label": "條件判斷資料",
        "description": "員工、部門、月薪、績效與出勤資料，可直接練條件判斷、複合條件與固定值對應。",
        "href": "practice-data/p1-03-if-logic.csv",
        "format": "CSV",
        "fileName": "p1-03-if-logic.csv"
      }
    ]
  },
  "P2-01": {
    "title": "條件統計練習資料",
    "intro": "這課的重點是依部門、年資、績效條件去加總或計數。先用同一張資料表把 SUMIF / COUNTIF / AVERAGEIF 系列練順。",
    "files": [
      {
        "label": "條件統計資料",
        "description": "可直接練 SUMIF / COUNTIF / AVERAGEIF / SUMIFS / COUNTIFS。",
        "href": "practice-data/p2-01-conditional-stats.csv",
        "format": "CSV",
        "fileName": "p2-01-conditional-stats.csv"
      }
    ]
  },
  "P2-02": {
    "title": "查找比對練習資料",
    "intro": "這份資料最適合練 XLOOKUP、VLOOKUP、INDEX+MATCH 與找不到資料的錯誤處理。先把「查找值、查找欄、回傳欄」的手感建立起來。",
    "files": [
      {
        "label": "產品主檔資料",
        "description": "產品代碼、名稱、類別、單價與庫存，可拿來練跨表查找、反向查找與 #N/A 處理。",
        "href": "practice-data/p2-02-product-catalog.csv",
        "format": "CSV",
        "fileName": "p2-02-product-catalog.csv"
      }
    ]
  },
  "P3-02": {
    "title": "文字與日期清理練習資料",
    "intro": "這份資料刻意保留常見的髒文字、訂單編碼、Email 與文字日期，適合練 TRIM/CLEAN/SUBSTITUTE、LEFT/MID/FIND、DATE 與 EOMONTH。",
    "files": [
      {
        "label": "文字日期資料",
        "description": "員工基本資料，適合練不可見空白清理、訂單號拆解、Email 擷取、日期轉換與格式輸出。",
        "href": "practice-data/p3-02-text-date-cleanup.csv",
        "format": "CSV",
        "fileName": "p3-02-text-date-cleanup.csv"
      }
    ]
  },
  "P4-01": {
    "title": "動態陣列練習資料",
    "intro": "這張產品表可以直接拿來練 FILTER、SORT、UNIQUE、SEQUENCE 與 D2# 溢出範圍。重點是感受一個公式吐出整片結果，並會處理空結果與 #SPILL!。",
    "files": [
      {
        "label": "動態陣列資料",
        "description": "產品、類別、單價、銷量與評分，適合練多條件 FILTER、動態排序、唯一清單與溢出引用。",
        "href": "practice-data/p4-01-dynamic-array-catalog.csv",
        "format": "CSV",
        "fileName": "p4-01-dynamic-array-catalog.csv"
      }
    ]
  },
  "P4-02": {
    "title": "進階函式練習資料",
    "intro": "這份資料適合練 LET、LAMBDA、AGGREGATE、SWITCH 與 INDIRECT / OFFSET 的風險判斷。先把長公式重構成可讀段落，再決定哪些邏輯值得封裝。",
    "files": [
      {
        "label": "進階函式資料",
        "description": "員工年度業績資料，適合練 LET 重構、LAMBDA 封裝、AGGREGATE 忽略錯誤與 SWITCH 分支判斷。",
        "href": "practice-data/p4-02-advanced-functions.csv",
        "format": "CSV",
        "fileName": "p4-02-advanced-functions.csv"
      }
    ]
  },
  "P1-01": {
    "title": "快捷鍵演練資料",
    "intro": "這課不只是一張清單，所以我另外準備了鍵盤演練題。可以直接下載後照著一輪輪做，把常用操作練成反射動作。",
    "files": [
      {
        "label": "快捷鍵演練清單",
        "description": "日常最常用的 8 個 Mac Excel 操作情境，可照順序做一輪暖身。",
        "href": "practice-data/p1-01-keyboard-drills.csv",
        "format": "CSV",
        "fileName": "p1-01-keyboard-drills.csv"
      }
    ]
  },
  "P2-03": {
    "title": "樞紐分析練習資料",
    "intro": "這份原始交易資料是給樞紐分析用的。欄位已經整理成乾淨格式，直接匯入 Excel 後就能練列、欄、值與日期群組。",
    "files": [
      {
        "label": "樞紐原始交易資料",
        "description": "跨部門、跨月份的交易流水帳，適合直接建立樞紐分析表。",
        "href": "practice-data/p2-03-pivot-sales-transactions.csv",
        "format": "CSV",
        "fileName": "p2-03-pivot-sales-transactions.csv"
      }
    ]
  },
  "P2-04": {
    "title": "條件式格式化練習資料",
    "intro": "這份 KPI 月報就是拿來練條件式格式化的。你可以直接用它做資料橫條、負成長警示和高客訴提醒。",
    "files": [
      {
        "label": "KPI 月報資料",
        "description": "同時包含業績、達成率、月增率與客訴數，適合做多層規則。",
        "href": "practice-data/p2-04-conditional-format-kpi.csv",
        "format": "CSV",
        "fileName": "p2-04-conditional-format-kpi.csv"
      }
    ]
  },
  "P3-01": {
    "title": "資料驗證練習資料",
    "intro": "這課最適合用一張輸入表加一張來源清單來練。我把表單和下拉來源分開準備好了，直接下載就能做清單驗證與防呆規則。",
    "files": [
      {
        "label": "新進人員表單",
        "description": "可直接拿來設定下拉選單、文字長度、日期與手機格式限制。",
        "href": "practice-data/p3-01-onboarding-form.csv",
        "format": "CSV",
        "fileName": "p3-01-onboarding-form.csv"
      },
      {
        "label": "驗證來源清單",
        "description": "做下拉選單用的來源資料，可當另一張工作表或命名範圍來源。",
        "href": "practice-data/p3-01-validation-lists.csv",
        "format": "CSV",
        "fileName": "p3-01-validation-lists.csv"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 2 份練習資料，比逐個點更省事。",
      "href": "practice-data/p3-01-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p3-01-practice-pack.zip"
    }
  },
  "P3-03": {
    "title": "圖表設計練習資料",
    "intro": "這份月報資料很適合練圖表。你可以先插一張預設圖，再自己刪雜訊、改標題，做成更像能交付的版本。",
    "files": [
      {
        "label": "月報圖表資料",
        "description": "月份、營收、目標與成長率都有，適合練柱狀圖、折線圖與組合圖。",
        "href": "practice-data/p3-03-chart-monthly-report.csv",
        "format": "CSV",
        "fileName": "p3-03-chart-monthly-report.csv"
      }
    ]
  },
  "P3-04": {
    "title": "表格與命名範圍練習資料",
    "intro": "這課不是要你背理論，而是把原始清單真的轉成 Table。這份資料適合直接拿來做表格、命名範圍和結構化參照。",
    "files": [
      {
        "label": "銷售明細表",
        "description": "適合先轉成 Table，再練結構化參照與自動擴展。",
        "href": "practice-data/p3-04-sales-table.csv",
        "format": "CSV",
        "fileName": "p3-04-sales-table.csv"
      }
    ]
  },
  "P3-05": {
    "title": "保護與安全練習資料",
    "intro": "這份範本很適合拿來練哪些欄位該開放、哪些欄位該鎖。你可以直接把輸入區、計算區和輸出區切開來做工作表保護。",
    "files": [
      {
        "label": "預算範本資料",
        "description": "適合練鎖定公式欄、保護工作表與區分輸入區。",
        "href": "practice-data/p3-05-budget-template.csv",
        "format": "CSV",
        "fileName": "p3-05-budget-template.csv"
      }
    ]
  },
  "P4-03": {
    "title": "假設分析練習資料",
    "intro": "這份資料適合練目標搜尋和情境反推。先把單價、成本、流量和轉換率放進模型，再去推要達標得改哪個數字。",
    "files": [
      {
        "label": "價格情境資料",
        "description": "可拿來做目標搜尋、情境分析與簡單 Solver 練習。",
        "href": "practice-data/p4-03-what-if-pricing.csv",
        "format": "CSV",
        "fileName": "p4-03-what-if-pricing.csv"
      }
    ]
  },
  "P4-04": {
    "title": "Power Query 練習資料",
    "intro": "這課最需要多個月份的原始檔，所以我直接幫你準備了可附加的 CSV。先把 1 到 3 月接起來，再新增 4 月做重新整理最有感。",
    "files": [
      {
        "label": "2026-01 原始 CSV",
        "description": "Power Query 匯入第一個月份資料。",
        "href": "practice-data/p4-04-sales-2026-01.csv",
        "format": "CSV",
        "fileName": "p4-04-sales-2026-01.csv"
      },
      {
        "label": "2026-02 原始 CSV",
        "description": "第二個月份資料，可練追加與轉換。",
        "href": "practice-data/p4-04-sales-2026-02.csv",
        "format": "CSV",
        "fileName": "p4-04-sales-2026-02.csv"
      },
      {
        "label": "2026-03 原始 CSV",
        "description": "第三個月份資料，可與前兩月附加成同一張表。",
        "href": "practice-data/p4-04-sales-2026-03.csv",
        "format": "CSV",
        "fileName": "p4-04-sales-2026-03.csv"
      },
      {
        "label": "2026-04 新增資料",
        "description": "做完流程後再匯入這份，最適合拿來測重新整理。",
        "href": "practice-data/p4-04-sales-2026-04.csv",
        "format": "CSV",
        "fileName": "p4-04-sales-2026-04.csv"
      },
      {
        "label": "Power Query 練習說明",
        "description": "建議的匯入與重新整理順序。",
        "href": "practice-data/p4-04-readme.txt",
        "format": "TXT",
        "fileName": "p4-04-readme.txt"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 5 份練習資料，比逐個點更省事。",
      "href": "practice-data/p4-04-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p4-04-practice-pack.zip"
    }
  },
  "P4-05": {
    "title": "資料模型練習資料",
    "intro": "資料模型要有多張表才有感，所以我直接幫你拆成事實表和維度表。最適合拿來練關聯、量值和模型樞紐。",
    "files": [
      {
        "label": "Fact Sales",
        "description": "事實表，記錄每一筆銷售。",
        "href": "practice-data/p4-05-fact-sales.csv",
        "format": "CSV",
        "fileName": "p4-05-fact-sales.csv"
      },
      {
        "label": "Dim Products",
        "description": "產品維度表，可跟 Fact Sales 透過產品ID 關聯。",
        "href": "practice-data/p4-05-dim-products.csv",
        "format": "CSV",
        "fileName": "p4-05-dim-products.csv"
      },
      {
        "label": "Dim Calendar",
        "description": "日曆維度表，可延伸成年、季、月切片分析。",
        "href": "practice-data/p4-05-dim-calendar.csv",
        "format": "CSV",
        "fileName": "p4-05-dim-calendar.csv"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 3 份練習資料，比逐個點更省事。",
      "href": "practice-data/p4-05-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p4-05-practice-pack.zip"
    }
  },
  "P5-01": {
    "title": "VBA 基礎練習資料",
    "intro": "這份原始資料很適合拿來錄巨集和寫第一個 VBA 腳本。先做簡單整理與格式化，再慢慢接到自動化流程。",
    "files": [
      {
        "label": "巨集錄製原始資料",
        "description": "適合練排序、加總、格式化與錄製第一個巨集。",
        "href": "practice-data/p5-01-vba-sales-raw.csv",
        "format": "CSV",
        "fileName": "p5-01-vba-sales-raw.csv"
      }
    ]
  },
  "P5-02": {
    "title": "VBA 進階練習資料",
    "intro": "進階 VBA 比較需要一點批次處理感，所以我準備了任務清單和事件紀錄。很適合練陣列、Dictionary 和錯誤處理。",
    "files": [
      {
        "label": "批次處理任務",
        "description": "可用來練迴圈、字典與批次狀態更新。",
        "href": "practice-data/p5-02-batch-tasks.csv",
        "format": "CSV",
        "fileName": "p5-02-batch-tasks.csv"
      },
      {
        "label": "事件紀錄",
        "description": "可練錯誤處理、事件偵測與執行紀錄彙總。",
        "href": "practice-data/p5-02-event-log.csv",
        "format": "CSV",
        "fileName": "p5-02-event-log.csv"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 2 份練習資料，比逐個點更省事。",
      "href": "practice-data/p5-02-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p5-02-practice-pack.zip"
    }
  },
  "P5-03": {
    "title": "VBA 實戰專案資料",
    "intro": "這裡準備一組比較像真實專案的資料，適合練輸入 → 處理 → 輸出 → 驗證的完整月報流程，而不是只跑單一巨集。",
    "files": [
      {
        "label": "Orders",
        "description": "訂單來源資料，可做月報彙總、Power Query 重新整理、PDF 批次輸出與交付紀錄。",
        "href": "practice-data/p5-03-orders.csv",
        "format": "CSV",
        "fileName": "p5-03-orders.csv"
      },
      {
        "label": "Customers",
        "description": "客戶主檔，可做補欄位與報表輸出。",
        "href": "practice-data/p5-03-customers.csv",
        "format": "CSV",
        "fileName": "p5-03-customers.csv"
      },
      {
        "label": "Config",
        "description": "專案設定表，可拿來練參數化與輸出控制。",
        "href": "practice-data/p5-03-project-config.csv",
        "format": "CSV",
        "fileName": "p5-03-project-config.csv"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 3 份練習資料，比逐個點更省事。",
      "href": "practice-data/p5-03-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p5-03-practice-pack.zip"
    }
  },
  "P5-04": {
    "title": "綜合挑戰資料包",
    "intro": "綜合挑戰就該像真的在接任務，所以我準備了一組多來源資料。你可以拿這些檔案自己決定該用公式、樞紐、Power Query 還是 VBA 解。",
    "files": [
      {
        "label": "Sales",
        "description": "主銷售資料，可做彙總、分析與報表。",
        "href": "practice-data/p5-04-capstone-sales.csv",
        "format": "CSV",
        "fileName": "p5-04-capstone-sales.csv"
      },
      {
        "label": "Targets",
        "description": "目標資料，可做達成率、差異分析與儀表板。",
        "href": "practice-data/p5-04-capstone-targets.csv",
        "format": "CSV",
        "fileName": "p5-04-capstone-targets.csv"
      },
      {
        "label": "Returns",
        "description": "退貨資料，可加入異常與淨額分析。",
        "href": "practice-data/p5-04-capstone-returns.csv",
        "format": "CSV",
        "fileName": "p5-04-capstone-returns.csv"
      },
      {
        "label": "挑戰說明",
        "description": "適合當這份資料包的任務說明。",
        "href": "practice-data/p5-04-challenge-brief.txt",
        "format": "TXT",
        "fileName": "p5-04-challenge-brief.txt"
      }
    ],
    "bundle": {
      "label": "本課資料包",
      "description": "一次下載這課的 4 份練習資料，比逐個點更省事。",
      "href": "practice-data/p5-04-practice-pack.zip",
      "format": "ZIP",
      "fileName": "p5-04-practice-pack.zip"
    }
  }
};
