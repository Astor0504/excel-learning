...
const formulas = [
  ...
  {name:"AVERAGE",cat:"math",lv:1,syntax:'=AVERAGE(範圍)',desc:"計算平均值",example:'=AVERAGE(B2:B10)  →  所有數字的平均',detail:"等於 SUM(範圍)/COUNT(範圍)。忽略空白與文字，但 0 會算進平均。",tip:"如果資料有 0 但不想算入，用 AVERAGEIF 排除",tags:["平均","統計"]},
  ...
  {name:"COUNTA",cat:"math",lv:1,syntax:'=COUNTA(範圍)',desc:"計算範圍內有多少個「非空白」",example:'=COUNTA(A1:A10)  →  計算有幾格有填內容',detail:"只要儲存格不是空白，就會算進去，包含文字、數字、日期、0、公式回傳值。",tip:"要算『有幾筆內容』通常用 COUNTA，不是 COUNT",tags:["計數","非空白","統計"]},
  ...
  {name:"LARGE / SMALL",cat:"math",lv:2,syntax:'=LARGE(範圍, k)  /  =SMALL(範圍, k)',desc:"找出第 k 大 / 第 k 小的值",example:'=LARGE(B2:B10, 3)  →  第 3 大',detail:"MAX / MIN 只會回傳最大或最小；要第 2 名、第 3 名、第 5 名就要用 LARGE / SMALL。",tip:"k=1 等於最大值或最小值；真正有差別的是 k>=2",tags:["排名","第N大","第N小"]},
  ...
  {name:"XLOOKUP",cat:"lookup",lv:2,syntax:'=XLOOKUP(查找值, 查找範圍, 回傳範圍, [找不到時])',desc:"新版查找主路線，取代多數 VLOOKUP 場景",example:'=XLOOKUP("P003", A:A, B:B, "查無資料")  →  直接查找',detail:"支援左右查找、多值回傳、找不到時的預設值。查找欄與回傳欄分開指定，不用手動數第幾欄。",tip:"如果用 365 / Excel 2021 以上，XLOOKUP 是首選",tags:["查找","新版","推薦"]},
  ...
  // Array/Dynamic (Lv.3)
  {name:"FILTER",cat:"array",lv:3,syntax:'=FILTER(範圍, 條件, [沒有結果時])',desc:"動態篩選，結果自動展開",example:'=FILTER(A1:D10, B1:B10="業務", "查無資料")',detail:'多條件用 * 相乘（AND）或 + 相加（OR）。第 3 參數可避免空結果時顯示 #CALC!。',tip:"取代輔助欄+進階篩選，365 最實用的新函式之一",tags:["篩選","動態","陣列","365"]},
...
const LESSON_CHEAT_MAP = {
  "P1-01": {cats:["shortcut"], title:"快捷鍵速查",
    ...
    highlights:["快捷鍵","⌘","⌃"]},
  "P1-02": {cats:["math"], title:"基礎函式速查",
    pick:["SUM","AVERAGE","COUNT","COUNTA","MAX / MIN","LARGE / SMALL","MEDIAN"],
    highlights:["SUM","AVERAGE","COUNT","COUNTA","MAX","MIN","MEDIAN","LARGE","SMALL"]},
  "P1-03": {cats:["condition"], title:"條件判斷速查",
    pick:["IF","IFS","AND / OR","SWITCH"],
    highlights:["IF","IFS","AND","OR","SWITCH"]},
  ...
  "P2-02": {cats:["lookup"], title:"查找函式速查",
    pick:["XLOOKUP","VLOOKUP","INDEX + MATCH","MATCH","IFERROR"],
    highlights:["XLOOKUP","VLOOKUP","INDEX","MATCH","IFERROR"]},
  ...
  "P3-02": {cats:["text","date"], title:"文字/日期函式速查",
    pick:["TRIM","LEFT / MID / RIGHT","FIND / SEARCH","SUBSTITUTE","TEXT","DATEDIF","WORKDAY","EOMONTH"],
    highlights:["TRIM","LEFT / MID / RIGHT","FIND / SEARCH","SUBSTITUTE","TEXT","DATEDIF","WORKDAY","EOMONTH"]},
  ...
  "P4-02": {cats:["advanced","condition"], title:"進階函式速查",
    pick:["LET","LAMBDA","AGGREGATE","SWITCH","INDIRECT","OFFSET"],
    highlights:["LET","LAMBDA","AGGREGATE","SWITCH","INDIRECT","OFFSET"]},
  ...
  "P5-03": {cats:[], kinds:["vba"], vbaCats:["file","pro"], title:"VBA 實戰範本",
    pick:[], pickVba:["匯出全部為 PDF","建立工作表目錄","自動月報表","錯誤處理框架","效能優化開關"],
    highlights:["Workbook","Worksheet","ErrHandler"]},
